...
class PandasDataSetLoader(DataSetLoader):
    ...
    def load_data_set_and_column_names(self, pathname):
        self.check_pathname(pathname)
        loaded_data = self.load_data(pathname)
        self.check_are_required_columns_inside(loaded_data, self.must_contained_columns)
        return self.extract_data_set_and_column_names(loaded_data, self.must_contained_columns)
    ...
    def check_are_required_columns_inside(self, loaded_data, must_contained_columns):
    ...
    def extract_data_set_and_column_names(self, loaded_data, must_contained_columns):
        column_names = loaded_data.columns
        data_set = loaded_data[column_names]
        other_column_names = self.get_other_column_names(column_names, must_contained_columns)
        return data_set, must_contained_columns, other_column_names

    def get_other_column_names(self, column_names, must_contained_columns):
        other_column_names = []
