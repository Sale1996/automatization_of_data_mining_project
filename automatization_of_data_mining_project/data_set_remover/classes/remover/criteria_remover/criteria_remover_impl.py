...
class CriteriaRemoverImpl(CriteriaRemover):
    def remove_and_return_updated_list(self, criteria_remover_data: DataForCriteriaRemove):
        ...
        for data_set_info in criteria_remover_data.data_sets_info:
            columns_to_check = self.__get_which_columns_to_check(criteria_remover_data, data_set_info)

            criteria_passed, \
            updated_data_set_info = self.__get_is_criteria_passed_and_updated_data_set_object(columns_to_check,
    ...
    def __get_is_criteria_passed_and_updated_data_set_object(self, columns_to_check, criteria_remover_data,
                                                           criteria_validators, data_set_info):
        criteria_passed = True
        for column_name in columns_to_check:
            is_column_passed_by_criteria = self.__column_check_by_criteria(column_name, criteria_remover_data,
            ...
            if not is_column_passed_by_criteria:
                ...
                if column_name in data_set_info.other_column_names:
                    ...
                    if non_must_contained_columns:
                        data_set_info = self.__remove_column_from_data_set(column_name, data_set_info,
    ...
    def __remove_column_from_data_set(self, column_name, data_set_info, non_must_contained_columns):
    ...
    def __column_check_by_criteria(self, column_name, criteria_remover_data, criteria_validators, data_set):
    ...
    def __get_which_columns_to_check(self, criteria_remover_data, data_set_info):
        if criteria_remover_data.columns_to_include:
            columns_to_check = criteria_remover_data.columns_to_include
        else:
            columns_to_check = self.__get_colums_exluded_columns_to_exclude(data_set_info.data_set.columns.tolist(),
                                                                          criteria_remover_data.columns_to_exclude)
        return columns_to_check

    def __get_colums_exluded_columns_to_exclude(self, all_columns, columns_to_exclude):
